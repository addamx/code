 ...
 */
  ...
  $.widget = function(name, base, prototype) {
	...
	// 如果有多个prototype, 则用$.extend合并
    if ($.isArray(prototype)) {
      prototype = $.extend.apply(null, [{}].concat(prototype));
    }

    // 创建一个伪类负责返回是否包含组件数据 $('#test:switch-class')
    ...
    // 如果第一个name.namespace是已经声明过的wiget则在会被继承
    ...
    constructor.prototype = $.widget.extend(
      basePrototype,
      {
        // 如果是已定义过的wiget则使用原来widgetEventPrefix
    ...
    if (existingConstructor) {
      ...
      // 将当前的constructor保存进父的_childConstructors属性, 以便以后同步父widget的修改.
  ...
  // 递归将arg终所有非plainObject的数据拷贝进target
        ...
        if (!this.length && options === "instance") {
          ...
          this.each(function() {
            ...
            if (methodValue !== instance && methodValue !== undefined) {
              returnValue =
                methodValue && methodValue.jquery   //methodValue.jquery 返回jquery版本号, 表示当前methodValue是一个jquery对象, .get()返回DOM对象
  ...
  $.Widget.prototype = {
    ...
    // 将包含事件当前elm的classesElementLookup中移除它
